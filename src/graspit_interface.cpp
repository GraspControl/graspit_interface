...
bool GraspitInterface::SetRobotDOFForces(graspit_interface::SetRobotDOFForces::Request &request,
                                         graspit_interface::SetRobotDOFForces::Response &response)
{
    if (graspitCore->getWorld()->getNumRobots() <= request.id) {
        ...
        for(int i=0; i < graspitCore->getWorld()->getHand(request.id)->getNumDOF(); i++) {
            graspitCore->getWorld()->getHand(request.id)->getDOF(i)->setForce(request.dof_forces.data()[i]);
        }
        response.result = response.RESULT_SUCCESS;
        return true;
    }
